...
export default function CarnivalDisplay() {
  // 1. State to track if the page has loaded (prevents hydration errors)
  ...
  // 2. Automatically generate the correct link for Vercel
  ...
  return (
    <div className="min-h-screen bg-[#1a0b2e] text-white flex items-center justify-center p-8 relative overflow-hidden font-sans">
      ...
      {/* BACKGROUND: Carnival Lights Effect (Purple & Pink) */}
      ...
      <div className="z-10 w-full max-w-6xl grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
        ...
        <div className="flex flex-col items-center text-center">
          <motion.div initial={{ y: -50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="mb-8">
            ...
            <h2 className="text-4xl font-bold text-white mt-2 tracking-tighter transform -rotate-2">2026</h2>
          </motion.div>
          ...
          >
            <div className="bg-white p-4 rounded-2xl">
              {/* Only show QR when mounted to be safe */}
              {mounted && registrationLink ? (
                <QRCode value={registrationLink} size={220} />
              ) : (
                <div className="w-[220px] h-[220px] bg-white/20 animate-pulse rounded-lg"></div>
              )}
            </div>
          </motion.div>
          
          <p className="mt-8 text-2xl font-bold animate-bounce text-cyan-300">Scan for Entry Pass 🎟️</p>
        </div>
...
// Helper Component for List Items (Restored your purple text styles)
